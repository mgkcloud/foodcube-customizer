...
export const DraggableBottomSheet: React.FC<DraggableBottomSheetProps> = ({
  ...
  const finalIsOpen = controlledIsOpen !== undefined ? controlledIsOpen : isOpen;
  ...
  return (
    <>
      ...
      <div
        ref={sheetRef}
        className={cn(
          'fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl rounded-t-3xl shadow-2xl z-50 transition-transform duration-300 ease-out lg:hidden',
          finalIsOpen ? 'translate-y-0' : 'translate-y-[calc(100%-8rem)]'
        )}
        style={{
          transform: isDragging
            ...
            : undefined,
          maxHeight: '85vh',
          boxShadow: '0 -10px 40px -10px rgba(0, 0, 0, 0.15), 0 -4px 16px -4px rgba(0, 0, 0, 0.1)'
        }}
      >
        {/* Extra Large Drag Handle Area */}
        <div
          className="flex flex-col items-center pt-6 pb-2 cursor-grab active:cursor-grabbing"
        ...
        >
          ...
          <div className="text-xs text-gray-400 font-medium mb-4">Swipe for details</div>
        </div>
        ...
        <div
          className={cn(
            'overflow-y-auto px-6 pb-6 transition-all duration-300',
            finalIsOpen ? 'max-h-[70vh] opacity-100' : 'max-h-0 opacity-0'
          )}
        >
          {children}
        </div>

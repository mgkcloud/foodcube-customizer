...
export const Grid: React.FC<GridProps> = ({
    ...
    useEffect(() => {
        if (setHasInteracted && !hasInteractedLocal) {
            ...
            const hasCubeInGrid = grid.some((row) =>
                row.some((cell) => cell.hasCube),
            );
            if (hasCubeInGrid) {
    ...
    const handleCellClick = (rowIndex: number, colIndex: number) => {
        ...
        console.log(
            `Grid cell clicked: [${rowIndex}, ${colIndex}], currently has cube: ${grid[rowIndex][colIndex].hasCube}`,
        );
            ...
            <div className="grid grid-cols-2 gap-0.5 absolute inset-0">
                {subgrid.map(({ subgridRow, subgridCol }) => (
                    <div
                        ...
                        onClick={() =>
                            console.log(
                                `Subgrid selected: ${subgridRow}, ${subgridCol}`,
                            )
                        }
                    />
                ))}
    ...
    return (
        <div className="relative grid grid-cols-3 gap-1.5 bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-5 md:p-6 rounded-2xl shadow-lg z-10">
            {!hasInteractedLocal && (
            ...
            {grid.map((row, rowIndex) =>
              ...
              border-2 sm:border-3 border-transparent rounded-xl
